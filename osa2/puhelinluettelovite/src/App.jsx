...
const Filter = ({ filterName, filterPerson }) => {
  return (
    <div>
      filter shown with <input value={filterName} onChange={filterPerson} />
    </div>
  );
};
...
const Button = ({ id, deletePerson }) => {
  return (
    <button value={id} onClick={deletePerson}>
      delete
    </button>
  );
};
    ...
    if (!newName) {
      ...
      if (personsName.includes(newName)) {
        ...
        if (
          ...
          personService.update(personId, newPerson).then((returnedPerson) => {
            setPersons(
              persons.map((person) => {
